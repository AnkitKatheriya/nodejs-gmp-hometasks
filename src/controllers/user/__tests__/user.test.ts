...
describe(UserController.name, () => {
  ...
  beforeAll(async () => {
    const response = await request(app)
      .get('/api/login/')
      .send(
        { username: 'test-user', password: 'test-user-psw' },
  ...
  describe('getAllUsers', () => {
    ...
    it('should return BAD_REQUEST and correct error message if login field not provided', async () => {
      ...
      expect(response.error.text).toEqual(
        'Error validating request body. "login" is required.'
      );
    });
    it('should return BAD_REQUEST and correct error message if password is not in correct format', async () => {
      const response = await request(app)
        ...
        .set('authorization', token)
        .send({
          login: 'automated-user-1',
          password: 'Automated_user1',
          age: 45,
          isDeleted: false,
        })
        .expect(HttpStatus.BAD_REQUEST);
      expect(response.error.text).toEqual(
        'Error validating request body. "password" with value "Automated_user1" fails to match the required pattern: /^(?=.*[0-9])(?=.*[a-zA-Z])([a-zA-Z0-9]+)$/.'
      );
    });
    it('should return BAD_REQUEST and correct error message if age is less than 4', async () => {
      const response = await request(app)
        ...
        .send({
          login: 'automated-user-1',
          password: 'Automateduser1',
          age: 2,
          isDeleted: false,
        })
        .expect(HttpStatus.BAD_REQUEST);
      expect(response.error.text).toEqual(
        'Error validating request body. "age" must be larger than or equal to 4.'
      );
    });
    it('should return BAD_REQUEST and correct error message if age is greater than 130', async () => {
      ...
      expect(response.error.text).toEqual(
        'Error validating request body. "age" must be less than or equal to 130.'
      );
    });
