...
class UserController {
  ...
  getAllUsers = async (
    req: ValidatedRequest<IAutoSearchUserSchema>,
    res: Response,
    next: NextFunction
  ) => {
    try {
      const { loginSubstring, limit } = await req.body;
      const autoSuggestedUsers = await this.userService.findAll(
        loginSubstring,
        limit
      );
      res.status(StatusCodes.OK).send({ data: autoSuggestedUsers });
    } catch (error) {
      next(error);
    }
  };
