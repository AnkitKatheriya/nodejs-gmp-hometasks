...
const groupCreateSchema = groupSchema.fork(['name', 'permissions'], schema =>
  schema.required()
);

const groupUpdatedSchema = groupSchema.fork(['name', 'permissions'], schema =>
  schema.required()
);

const groupDeleteSchema = groupSchema.fork(['id'], schema => schema.required());
